...
def get_logger(logpath, filepath, package_files=[], displaying=True, saving=True, debug=False):
    ...
    for f in package_files:
        logger.info(f)
        with open(f, "r", encoding="utf8") as package_f:
    ...
    def apply(self):
        if len(self.shadow_params) == 0:
            ...
            with torch.no_grad():
                ...
                source_list = []
                for name, param in self.module.named_parameters():
                    target_list.append(self.shadow_params[name])
                    source_list.append(param.data)
                torch._foreach_mul_(target_list, self.decay)
                torch._foreach_add_(target_list, source_list, alpha=1 - self.decay)
