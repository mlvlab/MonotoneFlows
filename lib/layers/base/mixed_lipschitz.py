    ...
    def build_clone(self):
        with torch.no_grad():
            if self.initialized:
                ...
                factor = torch.max(torch.ones(1, device=self.weight.device), self.scale / self.coeff)
                weight = self.weight / factor
                weight = weight.detach().requires_grad_(False)
            bias = self.bias.detach().requires_grad_(False) if self.bias is not None else None
            m = nn.Conv2d(self.in_channels, self.out_channels, self.kernel_size, self.stride, self.padding, bias=self.bias is not None, device=self.weight.device)
            m.weight.data.copy_(weight)
            if self.bias is not None:
                m.bias.data.copy_(bias)
            return m
    ...
    def build_jvp_net(self, x):
        ...
        with torch.no_grad():
            if self.initialized:
                ...
                weight = self.weight / factor
                weight = weight.detach().requires_grad_(False)
            m = nn.Conv2d(self.in_channels, self.out_channels, self.kernel_size, self.stride, self.padding, bias= None, device=self.weight.device)
            m.weight.data.copy_(weight)
            return m, self.forward(x).detach().clone()
