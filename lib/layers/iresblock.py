            ...
            if not self.exact_trace:
                ...
                if self.training and self.grad_in_forward:
                    g, logdetgrad = mem_eff_wrapper(
                        estimator_fn, self.nnet, x, n_power_series, power_series_coeff_fn, vareps, coeff_fn, self.training
                    )
                else:
                    x = x.requires_grad_(True)
                    g, logdetgrad = estimator_fn(self.nnet, x, n_power_series, power_series_coeff_fn, vareps, coeff_fn, self.training)
            else:
                ############################################
